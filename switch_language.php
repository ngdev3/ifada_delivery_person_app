<?php include 'header.php';?>

<header class="container-fluid nav-down">
	<div class="row">
		<nav class="navbar navbar-inverse">
		  <div class="container no-padding">
			<ul class="nav navbar-nav top_header text-center">
			  <li class="menu_icon "><a href="#"  ><img src="assets/img/icon/back.png" id="asdf" alt="menu icon"style="width: 13px;" /></a></li>
				<li class="page_name left">
					<a href="#" class="text-left ">
						<p>Switch Language</p>
					</a>
				</li>
			  
			</ul>
			
		  </div>
		</nav>
	
	</div>
	<!-- MOBILE-MENU-AREA END -->
</header>
<!-- END HEADER -->
	
<div class="container">
		<div class="login_form switches">
			<div class="form_container pt-10">
				<form class="">
				  <div class="col-xs-12">
					<!--<div class="addr_heading">-->
					<div class="">
						<div class="sec_title"><p>Select Language</p></div>
						  <div class="radio mb-20 mt-10">
							<input id="radio-1" name="radio" type="radio">
							<label for="radio-1" class="radio-label">English</label>
						  </div>
						  <div class="radio mb-20">
							<input id="radio-2" name="radio" type="radio">
							<label for="radio-2" class="radio-label">Arab</span></label>
						  </div>
					</div>
				  </div>
				  
				  <div class="submit-btn col-xs-12 text-center">
					<a href="#">OK</a>
				  </div>
					
				</form>
				



			</div>
		</div>
	</div>

	


<?php include 'footer.php';?>